		...
		go func() {
			...
			for {
				...
				case msg, ok := <-ch:
					...
					select {
					case <-ticker.C:
						log.Printf("[TRACE] %s: generated %dth msg of %d bytes(%d runes): %s", topic.ID(), totalCount.Load(), len(msg), utf8.RuneCountInString(msg), msg[:20])
					default:
						break
					}
	...
	for {
		...
		for _, v := range allTopics {
			...
			if batched > 0 || overflow > 0 || resend > 0 {
				log.Printf("[TRACE].  %s is not done yet: batched:%d, overflow:%d, resend:%dd", v.ID(), batched, overflow, resend)
				finished = false
				break
			}
		}
